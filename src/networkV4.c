...
static void batch_forward(const double *A, const double *W, const double *b,
    ...
    for (int r = 0; r < rows; r++){
        ...
        double br = b[r];
        // m is mini batch count, so if parallel most parallel I could get is number of mini batches
...
static void batch_grad(const double *D, const double *A,
    ...
    for (int r = 0; r < rows; r++){
        ...
        #pragma omp parallel for reduction(+:nabla_w[:rows*cols])  // reduction does the split and sum up thing, + initializes all row*col mem to 0, add up to nabla_w
        for (int i = 0; i < m; i++) {
