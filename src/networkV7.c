...
int evaluate(const Network *net, const Dataset *data){
    ...
    #pragma omp parallel reduction(+:correct)
    // split this into their own threads cuz tallying correct needs to be added
    // back at teh very end. 
    {
        ...
        if (input == NULL || output == NULL || scratch == NULL){
            ...
            // every image i's classification depends only on data->pixels[i], network weights, and private buffer scratch space
            // distributes loop across the threads. with 30 threads and 10k images, that's about 333 per thread. 
...
static void batch_forward(const double *A, const double *W, const double *b,
         double *Aout, int m, int rows, int cols){
    ...
    for (int r = 0; r < rows; r++){
        ...
        for (int i = 0; i < m; i++) {
            ...
            Aout[(size_t)i * rows + r] = sigmoid(sum + br);
        }
    }
}
...
void batch_backprop(const Network *net, const Dataset *data, const int *idx, int m,
    ...
    // Each 'i' owns its own row of A[0]
    // each ws->A[0] is a flat array holding the whole mini-batch of inpout images
    //              m x d doubles where d = 784 (pixels of one mnist image)
    //              iteration i=0 writes only from 0-783, 1 writes to 784-1567
    #pragma omp parallel for schedule(static)
    for (int i = 0; i < m; i++){
    ...
    // cant parallelize cuz ws gets set every time
    for (int l = 0; l <= L; l++){
